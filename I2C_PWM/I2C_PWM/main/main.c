...
#define LEDC_OUTPUT_IO          (2)  //(5) // Define the output GPIO // 
...
static void temp_to_led_task(void *arg) {
  ...
  while(1){
    ...
    if (temperature_value > initial_temp + 3*range) {
      // Decrease the intensity by setting the duty to 100%
      ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, LEDC_DUTY_100));
      // Update duty to apply the new value
      ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));
      last_duty=LEDC_DUTY_100;
    }
    else if (temperature_value > initial_temp + 2*range) {
      // Decrease the intensity by setting the duty to 75%
      ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, LEDC_DUTY_75));
      // Update duty to apply the new value
      ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));
      last_duty=LEDC_DUTY_75;
    }
    else if (temperature_value > initial_temp + range) {
      // Decrease the intensity by setting the duty to 50%
      ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, LEDC_DUTY_50));
      // Update duty to apply the new value
      ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));
      last_duty=LEDC_DUTY_50;
    }
      ...
      // Decrease the intensity by setting the duty to 0%
      ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, LEDC_DUTY_0));
      // Update duty to apply the new value
      ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));
      last_duty=LEDC_DUTY_0;
    }else{
      ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, last_duty));
      // Update duty to apply the new value
      ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));
    }
